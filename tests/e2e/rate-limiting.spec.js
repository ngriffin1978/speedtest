...
test.describe('Rate Limiting', () => {
  ...
  test('should decrement rate limit remaining on subsequent requests', async ({ request }) => {
    const response1 = await request.get('http://localhost:8888/health');
    const remaining1 = parseInt(response1.headers()['ratelimit-remaining'] || '0');

    const response2 = await request.get('http://localhost:8888/health');
...
test.describe('Security Headers', () => {
  ...
  test('should include security headers on Silver port', async ({ request }) => {
    const response = await request.get('http://localhost:8889/');
  ...
  test('should have consistent security headers across ports', async ({ request }) => {
    const goldResponse = await request.get('http://localhost:8888/');
    const silverResponse = await request.get('http://localhost:8889/');
