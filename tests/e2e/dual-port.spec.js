...
test.describe('Dual Port Architecture', () => {
  ...
  test('should detect Silver transport on port 8889', async ({ request }) => {
    const response = await request.get('http://localhost:8889/health');
  ...
  test('should return different transport types for same endpoint on different ports', async ({ request }) => {
    const goldResponse = await request.get('http://localhost:8888/api/test/info');
    const silverResponse = await request.get('http://localhost:8889/api/test/info');
  ...
  test('should include transport headers in all responses', async ({ request }) => {
    ...
    for (const endpoint of endpoints) {
      const goldResponse = await request.get(`http://localhost:8888${endpoint}`);
      const silverResponse = await request.get(`http://localhost:8889${endpoint}`);
  ...
  test('should have consistent data center ID across both ports', async ({ request }) => {
    const goldResponse = await request.get('http://localhost:8888/health');
    const silverResponse = await request.get('http://localhost:8889/health');
  ...
  test('should handle concurrent requests to both ports', async ({ request }) => {
    const promises = [
      request.get('http://localhost:8888/health'),
      request.get('http://localhost:8889/health'),
      request.get('http://localhost:8888/api/test/info'),
      request.get('http://localhost:8889/api/test/info'),
    ];
